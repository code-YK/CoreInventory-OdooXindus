...
def list_deliveries(db: Session, filter_status: Optional[str] = None) -> List[Operation]:
    query = db.query(Operation).filter(Operation.type == "delivery").options(
...
def create_delivery(source_location_id: UUID, customer: Optional[str], items: list, user_id: UUID, db: Session) -> Operation:
    reference = generate_reference("DEL", db)
...
def confirm_delivery(delivery_id: UUID, db: Session) -> Operation:
    operation = db.query(Operation).filter(Operation.id == delivery_id, Operation.type == "delivery").first()
    if not operation:
...
def done_delivery(delivery_id: UUID, db: Session) -> Operation:
    operation = (
        db.query(Operation)
        .filter(Operation.id == delivery_id, Operation.type == "delivery")
    ...
    # Check stock sufficiency and decrease
...
def cancel_delivery(delivery_id: UUID, db: Session) -> Operation:
    operation = db.query(Operation).filter(Operation.id == delivery_id, Operation.type == "delivery").first()
    if not operation:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Delivery not found")
